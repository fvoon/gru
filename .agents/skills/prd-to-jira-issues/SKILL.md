...
8. **Plan children**. Run `scripts/write_jira_children.py --slice-plan redlines/PLTPM-XXXXX.md --parent-key PLTPM-XXXXX`. Emits a deterministic 3-phase JSON action plan. Every Phase 1 (slice) `createJiraIssue` action automatically carries `additional_fields.customfield_12881` (Activity Type) sourced from `.agents/jira-conventions.md`. Override the project default for this PRD via `--activity-type "<value>"` (must match one of the conventions-allowed values).
9. **Execute**. For each action, call the resolved Atlassian MCP tool. Substitute `{{slice_<letter>_key}}` placeholders from previous `createJiraIssue` results. Phases run in order: createIssue per slice → Implement link parent→child → Blocks links per `depends_on:`. PLTPM's Jira automation creates the 5 ceremony Sub-tasks per Task/Technical-Story slice automatically; do not emit them.
