...
4. **Jira + Confluence bootstrap** — ✅ workflow statuses discovered (`To Do → next → In Progress → ...`); ✅ `Implement` link type confirmed; ✅ 4 repo-shaped Components created in PLTPM (`payment-platform`, `walletapi`, `infrastructure`, `spring-boot-starters`); ✅ Confluence `## Confluence` section in `.agents/jira-conventions.md` populated with real values: space key `PS1` (URL alias `payments`), space id `5015143175`, Spikes parent folder `15 - Spikes` (id `6711345211`, sibling of the existing numbered `NN - Section` folders under the Payments space homepage). `validate_spike_target.py` no longer emits `needs_conventions_bootstrap`.
...
6. **Narrative — glossary** — ⚠️ DRAFTED, pending PM/eng review: 7 entries in `knowledge/narrative/glossary.md` covering cross-repo term mismatches (`Transfer`↔`Transaction`, `Account`↔`Wallet`, Axon `Event`↔SQS `Message`), code-vs-business vocabulary (`Topup`), intra-repo same-name-different-thing (`Transfer` vs. `TransferAuthorization`; `TransferStep`/`stepId`/`uniqueTransactionId`), and ACH `Settled` vs. `Final`. Each carries `<!-- TODO -->` markers on bridging assumptions that need walletapi / payment-platform owner confirmation. Graphify god nodes were inspected but proved too generic (`Builder`, `Name`, `toString`) — entries were sourced from the three drafted flows instead, consistent with the "narrative complements graphify, doesn't duplicate it" rule.
...
10. **Skill: `prd-to-jira-issues`** — ✅ scaffolded, pending end-to-end demo: `.agents/skills/prd-to-jira-issues/` with `SKILL.md` (59 lines), `REFERENCE.md`, `EXAMPLES.md`, three deterministic Python scripts (`validate_parent.py`, `propose_slices.py`, `write_jira_children.py`), 108 unit tests + 4 goldens (raw + redlined slice plans, expected JSON action plans for single-repo and multi-repo elevated parents). Shared markdown helpers extracted to `.agents/skills/_lib/markdown.py` (write-a-prd's 97 tests still green after refactor). Human checkpoint at `redlines/<parent-key>.md` between propose and write. Real PLTPM end-to-end is gated on the `demo` todo.
11. **Skill: `spike-and-report`** — ✅ scaffolded, pending end-to-end demo: `.agents/skills/spike-and-report/` with `SKILL.md` (59 lines), `REFERENCE.md`, `EXAMPLES.md`, three deterministic Python scripts (`validate_spike_target.py`, `bootstrap_worktree.py`, `write_spike_report.py`), 179 unit tests + 2 goldens (canonical "transactional outbox" briefing + write-action-plan JSON, both byte-stable). 7-section report template with `TODO`-guarded validation and an in-script markdown→Confluence storage-format converter scoped to that template. Three-level Confluence parent walk (elevated PRD → configured Spikes parent → orphan Spikes parent), four-tier idempotency (existing redline / worktree / state-file / Confluence link / Done status), and `--reset-worktree` / `--update-existing` opt-outs for controlled re-runs. Throwaway worktree per repo with `push.default=nothing` belt+braces no-PR enforcement, soft 30-minute wall-clock + 30-turn LLM-self-policed budget, BUDGET_EXCEEDED stamping in the metadata footer. `_lib/markdown.py` reused (`write-a-prd` + `prd-to-jira-issues` tests still green: 97 + 108 = 205). `## Confluence` section in `.agents/jira-conventions.md` (schema_version bumped to 2 then 3; placeholders subsequently bootstrapped to real PLTPM values — see item 4). Real PLTPM end-to-end is gated on the `demo` todo.
12. **Skill: `ai-ready-check`** — ✅ scaffolded, pending end-to-end demo: `.agents/skills/ai-ready-check/` with `SKILL.md` (60 lines), `REFERENCE.md`, `EXAMPLES.md`, one deterministic Python script (`validate_ai_ready.py`, ~800 lines including docstring), 81 unit tests + 2 sentinels + 2 goldens (canonical pass and multi-fail action plans, both byte-stable). Six-item checklist (AC presence, blockers Done, exactly one canonical Component, Implement-link to a parent, status `next`, allowed issue type) with the AC check collapsed to "section present + non-empty" per locked decision 2D. Multi-phase state machine (`needs_fetch_ticket` → `needs_fetch_blockers` → terminal `ai_ready` / `not_ai_ready` / `no_change`) following the same pattern as `validate_parent.py` and `validate_spike_target.py`; per-blocker `getJiraIssue` fetch per locked 1A. Versioned-marker outcome diff (`<!-- ai-ready-check:v1 -->` + embedded outcome JSON) gives byte-stable comments and a no-op short-circuit on unchanged re-runs (locked 4B). Label edits use the merge-safe REST `update.labels[{add|remove}]` shape and are only emitted when the current state disagrees with the desired state. Sentinel-tested only `CANONICAL_COMPONENTS` + `AI_READY_LABEL` against `.agents/jira-conventions.md` per locked 3C; trusts Atlassian-stable globals (`Implement`, `Blocks`, `Done`, `next`) at runtime. New `## Labels` section in `.agents/jira-conventions.md` (schema_version bumped to 3) documents `ai-ready` as the AI gate and the per-skill rules for who may write it. `_lib/markdown.py` reused for AC section parsing (`write-a-prd` + `prd-to-jira-issues` + `spike-and-report` tests still green: 97 + 108 + 179 = 384). Real PLTPM end-to-end is gated on the `demo` todo.
